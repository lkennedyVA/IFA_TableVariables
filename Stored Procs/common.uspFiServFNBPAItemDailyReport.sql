USE [IFA]
GO
/****** Object:  StoredProcedure [common].[uspFiServFNBPAItemDailyReport]    Script Date: 2/18/2025 9:52:51 AM ******/
SET ANSI_NULLS ON
GO
...
/****************************************************************************************
    Name: [common].[uspFiServFNBPAItemDailyReport]
    Description: This procedure reports on FNBPA Item activity for the date range.
...
*****************************************************************************************/
ALTER PROCEDURE [common].[uspFiServFNBPAItemDailyReport](
    @piOrgId INT,
...
END;
    ...
    DROP TABLE IF EXISTS #FiServItemDailyReportFNBPA;
    DROP TABLE IF EXISTS #tblFiServDetailFNBPA;

    -- Create temp table for hierarchical organization data
    CREATE TABLE #FiServItemDailyReportFNBPA (
        LevelId INT,
        ParentId INT,
    ...
    CREATE TABLE #tblFiServDetailFNBPA (
    ...
    INSERT INTO #FiServItemDailyReportFNBPA (LevelId, ParentId, OrgId, OrgCode, OrgName, ExternalCode, TypeId, [Type], StatusFlag, DateActivated, ChannelName)
    SELECT 
    ...
    INSERT INTO #tblFiServDetailFNBPA (
    ...
    INNER JOIN #FiServItemDailyReportFNBPA dol ON p.OrgId = dol.OrgId
