USE [IFA]
GO
/****** Object:  StoredProcedure [common].[uspFiServTFBItemDailyReport]    Script Date: 2/18/2025 9:54:47 AM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
    SELECT 
        ...
        CAST(NULL AS NVARCHAR(3)) AS OnUs;
    RETURN;
END;

    -- Drop temp tables if they exist
    DROP TABLE IF EXISTS #FiServItemDailyReportTFB;
    DROP TABLE IF EXISTS #tblFiServDetailTFB;

    -- Create temp table for hierarchical organization data
    CREATE TABLE #FiServItemDailyReportTFB (
        LevelId INT,
        ParentId INT,
    ...
    CREATE TABLE #tblFiServDetailTFB (
    ...
    INSERT INTO #FiServItemDailyReportTFB (LevelId, ParentId, OrgId, OrgCode, OrgName, ExternalCode, TypeId, [Type], StatusFlag, DateActivated, ChannelName)
    SELECT 
    ...
    INSERT INTO #tblFiServDetailTFB (
        ...
        ClientResponse, ItemAmount, NotEligible, CarveOut, SmallItemFlag, OnUs
    )
    SELECT TOP 1000000 
        p.DateActivated,
        p.OrgId,
        ...
        CONVERT(NVARCHAR(1), CASE 
            ...
            THEN 1 ELSE 0 END),
        CONVERT(NVARCHAR(3), CASE 
            WHEN [common].[ufnOnUsRoutingNumber]([common].[ufnOrgClientId](p.OrgId), py.RoutingNumber) = 1 THEN 'Yes' ELSE 'No' END)
    ...
    INNER JOIN #FiServItemDailyReportTFB dol ON p.OrgId = dol.OrgId
